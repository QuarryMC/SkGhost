    ...
    companion object {
        init {
            Skript.registerExpression(
                ...
                "[the] blocks in %view% (of|in) %stage%",
                "all blocks in %view% (of|in) %stage%"
            )
        }
    }

    override fun toString(event: Event?, debug: Boolean): String {
        return "Blocks in view expression with expression view: ${
            view.toString(
                event,
                debug
            )
...
//        return stage.views[view.name]!!.blocks.keys.map { p -> p.toLocation(stage.world) }.toTypedArray()
        return view.getAllBlocksInBound().keys.map { it.toLocation(stage.world) }.toTypedArray()
    }
